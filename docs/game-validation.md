# 영구 운동·밤길 서사 검증 — 2026-09-12

`bash scripts/test-game.sh --render`의 10개 headless suite와 화면 렌더링을 통과했다. 세 지역 × 일반/도전의 전체 진행도 각각 보스 처치로 완료했다. 추가된 `training_test.gd`는 3분 패배의 츄르 6·통조림 1 정산, 첫 운동 두 개 구매 후 HP 105·XP 반경 59.16, 모든 고양이 공통 적용, 회복 상한, 저장 이관, 정산 중복 방지, 저장 실패 롤백, 이미 정산한 산책의 이어하기 차단을 확인한다. 보스 생성은 시작점뿐 아니라 맵 가장자리와 여러 내부 위치에서 주변 여유 공간을 검사했다.

화면은 `build/training-ready.png`, `build/training-done.png`, `build/results.png`, `build/victory.png`, `build/story-2.png`를 포함해 검토했다. 운동 카드의 비용·보유량·부족 상태, 실제 정산된 패배/승리 보상, 주인과의 재회 아트를 확인했다.

추가 캠페인 검사 중 기존 자동 봇이 담장 반대편의 진화 상자를 향해 매 프레임 좌우로 왕복하는 문제가 드러났다. 벽을 만났을 때 우회 방향을 잠시 유지하도록 테스트 봇을 수정했다. 경험치 수치와 최소 성장량 검증은 낮추지 않았다. 봇은 여전히 무적이며 사람 난도·재미·모바일 성능 검증을 대신하지 않는다. 장면별 추가 컷과 전용 환경음은 스토리보드의 제작 디렉션으로 남아 있다.
...
## 2026-09-12 — 글자 정렬, 첫 플레이 안내와 화면 방향

- 버튼의 상태별 위아래 여백을 통일하고 배지·설정 행·상점·쉼터·전투 상태·결과 카드의 글자를 컴포넌트 높이 기준으로 세로 중앙 정렬했다. UI/햅틱/렌더링 검사를 통과한 뒤 iPhone 설치·실행에 성공했다(커밋 `c8d5e51`).
- 결과 화면의 청록색 전체 덮개를 중립적인 검정 반투명으로 바꾸고 기존 골목 배경을 유지한다. 결과 카드도 공통 보라색 팔레트를 사용하며 전투 HUD를 숨긴다.
- 화면 문구의 ‘괴이’를 일반 적은 ‘유령’, 강한 일반 적은 ‘강한 유령’으로 바꿨다. 보스는 기존 보스 이름을 유지한다. 공포·기척·탄막 등 설명도 겁주기·유령·날아오는 공격처럼 읽기 쉬운 표현으로 정리했다.
- 첫 산책 전 4쪽 안내와 일시정지의 ‘게임 방법’을 추가했다. 안내 중 전투 정지, 완료 상태 저장/재로드, 재열기 후 일시정지 복귀를 UI 테스트로 확인했다.
- 밥그릇은 실제 회복 반경 22와 같은 초록 원을 표시하고, 진입 시 ‘회복 중 · 초당 +2’ 또는 ‘체력 가득!’을 표시한다. 원 안의 초당 2 회복과 원 밖의 회복 중단을 검증했다.
- 존재하지 않던 발자국 길 안내를 없애고 물건에 노란 원·방향 화살표를 표시한다. 적과 배경 위에 그려 가려짐을 줄였다. 안내와 발견 기록에서 밥을 주던 사람을 찾는 목적, 수집이 선택 사항이라는 점을 설명한다.
- `bash scripts/test-game.sh --render` 전체 통과. 이후 최종 문구 수정 후 렌더링을 다시 검사했다. 안내 4쪽·밥그릇/물건 표시·결과 화면을 이미지로 확인했다.
- 화면 방향을 `SCREEN_SENSOR_LANDSCAPE`로 변경했다. [Godot 공식 방향 열거형](https://docs.godotengine.org/en/4.6/classes/class_displayserver.html#enum-displayserver-screenorientation)을 확인했으며 생성된 iOS Info.plist에 LandscapeLeft/Right 두 방향만 포함된다. 물리적으로 휴대폰을 손으로 돌리는 검증은 수행하지 않았다.
- 최종 변경으로 iPhoneOS 서명 빌드와 연결된 iPhone 설치가 성공했다(`build/iphone-guide-final-install.log`). 빌드된 앱의 지원 방향도 가로 왼쪽/오른쪽으로 확인했다. 자동 실행은 기기 잠금 때문에 iOS가 `Locked` 오류로 거부했다. 따라서 이번 버전의 실기기 실행·회전 확인은 잠금 해제 후 남아 있다. 최종 UI 검사도 `UI_TEST_OK`다.
