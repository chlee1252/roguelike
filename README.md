...
**최신 변경:** 귀여운 고양이 4종과 배경 테마 4종, 공용 토큰 해금 상점. [상점 구조·테스트 방법·실제 결제 연결 상태](docs/token-shop.md)를 확인하세요. **현금 결제는 스토어·인증·영수증 검증 연결 전이라 비활성 상태**입니다. 기본 고양이와 기본 배경은 바로 플레이할 수 있습니다.

가상 토큰으로 해금을 미리 시험하려면 `godot --path . -- --shop-preview`를 실행하세요. 디버그 전용이며 일반 구매 내역과 분리됩니다. [생활형 무기·보스 클리어 규칙](docs/cute-theme.md)은 그대로 유지합니다.
