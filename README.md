...
**고양이·배경 상점:** 고양이 4종과 배경 테마 4종은 공용 유료 토큰으로 해금하는 구조입니다. [상점 구조·테스트 방법·결제 연결 상태](docs/token-shop.md)를 확인하세요. **실제 현금 결제는 스토어·인증·영수증 검증 연결 전이라 비활성 상태**입니다. 기본 고양이와 기본 배경은 바로 플레이할 수 있습니다.

가상 토큰으로 외형 해금을 시험하려면 `godot --path . -- --shop-preview`를 실행하세요. 디버그 전용이며 일반 외형 구매 내역과 분리됩니다. 전투 콘텐츠 해금은 쉼터의 간식을 사용합니다.
